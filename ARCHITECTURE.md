...
- ✅ **Voting Portal** (`src/pages/miniapps/VotingPortal.tsx`) — Dictators create polls (title, description, mode, multi-line options, optional close date, optional hide-results-until-close). Three voting modes via `src/components/voting/PollCard.tsx`: single-choice (radio), multi-select (checkboxes), ranked-choice (click to rank). Tallying logic in `src/lib/voting.ts` — Borda count for ranked. Live results bar with crown on winner(s). Each ballot is a row in `votes_ballots`; submitting "changes my vote" wipes prior rows for that voter+poll then inserts the new ones. Dictators see audit trail (who voted what), can close/reopen, and can delete polls. Banner auto-posted via client insert when a poll is created; auto-rescinded when the poll is manually closed.
- ✅ **Meeting Scheduler** (`src/pages/miniapps/MeetingScheduler.tsx`) — Anyone (not just Dictators) can propose meetings with multiple candidate options (date/time + location + label). Three response modes via `src/components/meetings/ProposalCard.tsx`: availability (per-option ✅/🤔/❌), voting (per-option vote toggle), ranked (click to rank with Borda count). Tallying logic in `src/lib/meetings.ts`. Schema reworked from initial setup: `meeting_proposals` → `meeting_options` → `meeting_responses` (mirrors voting tables). Proposers and Dictators see "still waiting on" panel listing family members who haven't responded; can close/reopen/delete. Banner auto-posted on create; auto-rescinded on close.
- ✅ **NCAA Tournament Pool** (`src/pages/miniapps/NCAAPool.tsx`) — Per-year pool tracking. Three tabs: This year (live standings table + bar chart, Dictator can edit/finalize), By year (year-tab picker), All-time (per-bracket aggregate: years entered, wins, last places, best finish, high score). Schema additions in `src/lib/ncaaPool.ts` + `ncaa_pool_years` table for per-year metadata (winner/loser at finalize). Dictator's StandingsEditor (`src/components/ncaa/StandingsEditor.tsx`) accepts paste-from-spreadsheet bulk input ("Mom 142") with auto-matching to family profiles, plus per-row inline edit. Finalizing a year posts a 🏀 banner and locks in winner/loser for the Virtual Plaques mini-app.
- ✅ **New Year's Eve Predictions** (`src/pages/miniapps/NewYearsPredictions.tsx`) — Year-selector page. Each family member submits questions for a `season_year`; everyone makes private predictions; on the next NYE a Dictator reveals the answer and scores each prediction ✓/✗. Multi-year leaderboard with per-year breakdown and hit rate. Sealed predictions hidden by UI (honor system) until reveal — schema permissive. Tally logic in `src/lib/nye.ts`.
- ✅ **Virtual Plaques** (`src/pages/miniapps/VirtualPlaque.tsx`) — Wall view of family awards. Auto-computes plaques from finalized NCAA Pool years (winner + loser) and scored NYE Predictions (top + bottom of those who participated). Custom plaques added via Dictator-only form, persisted in `plaques` table. Computation logic in `src/lib/plaques.ts`. Visual `Plaque` component (`src/components/plaques/Plaque.tsx`) renders three variants: brass for winners, dark wood for losers, amber for custom. Filterable by type / year / recipient.

### Status: feature-complete 🎉

All nine mini-apps from the original sketch are built. Remaining work is iteration: refining behavior, adding photo uploads (Supabase Storage), tightening RLS where we currently rely on UI-level honor system (NYE sealed predictions), and any new mini-apps that come up.
...
**Future uses:** NCAA round complete (NCAA Pool), manual heads-up posts ("Aunt Marie is in the hospital"). Each just inserts an announcement row with its own `source` and a relevant `link_path`. Voting Portal and Meeting Scheduler both already do this on create (and rescind on close).
