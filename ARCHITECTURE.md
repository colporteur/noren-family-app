...
**Future uses:** new meeting proposal (Meeting Scheduler), NCAA round complete (NCAA Pool), manual heads-up posts ("Aunt Marie is in the hospital"). Each just inserts an announcement row with its own `source` and a relevant `link_path`. Voting Portal already does this on poll create (and rescinds on close).
